...
def classify_intent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    final_intent_classification_prompt = intent_classification_prompt.format(
        user_input=state['user_input'],
        conversation_history=history
        )

    response = structured_llm.invoke(final_intent_classification_prompt)


    next_step = "qa"

    if "summarization" in response.intent_type:
        next_step = "summarization"
    elif "calculation" in response.intent_type:
...
def qa_agent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    structured_llm_answer = result['structured_response']

    final_llm_answer = f"{structured.answer} \nSources: {structured.sources}. \nConfidence: {structured.confidence}"

    return {
        "messages": AIMessage(content=final_llm_answer),
...
def summarization_agent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    final_llm_answer = f"{structured.summary} \nKey points: {structured.key_points}. \nDocuments summarized: {structured.document_ids}"
...
def update_memory(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    This function maintains conversation context and 
    tracks document references across turns.
    """
